...
class BackupScheduler:
    ...
    def start(self):
        ...
        if not self.s3_manager:
            logger.error("S3Manager not configured - backups DISABLED")
            return
        ...
        try:
            ...
            logger.error(f"✗ Failed to start BackupScheduler: {e}", exc_info=True)
    ...
    def _backup_job(self):
        ...
        try:
            ...
            if not self.vectorstore_path.exists():
                logger.error(f"Vectorstore path not found: {self.vectorstore_path}")
                return
            ...
            try:
                ...
                logger.error(f"Tarfile compression failed: {tar_error}", exc_info=True)
                return
